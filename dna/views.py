...
def match_dna_sample(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            matching_offset = get_best_offset(ref_dna, dna_sample)
            diffs = get_diffs(ref_dna, dna_sample, matching_offset,
                              show_index_numbers=True)
            changes = get_triplets(ref_dna, dna_sample, matching_offset,
                                   different_only=True)
